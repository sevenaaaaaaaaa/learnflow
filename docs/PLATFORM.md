...
### 2026-09-18 · 后台 UI 与性能检测（提交 `8a1d591`）

用浏览器逐页实测（看板/课程编辑/营收/AI/设置/自进化/策略库/会员/素材/内容库/学员 + 390px 移动端）。
- **UI/交互**：侧栏 22 项→按 概览/内容/教学/运营/系统 分组；`.lf-row` 内 btn/label/form 不再被拉伸（修看板操作行、自进化操作列、会员表格错位）；修 `.lf-form-card{margin:auto}` 在 grid 中收缩（营收页卡片 124px→613px）；表格状态列 nowrap + 列宽约束；营收页 2fr/1fr 栅格；移动端侧栏转顶部横向滚动；空态高度收敛
- **性能**：后台各页 2–80ms（本地），自体检 8ms@2 门课；无慢点，未加缓存
- 资源版本号提升以破 Service Worker 缓存
